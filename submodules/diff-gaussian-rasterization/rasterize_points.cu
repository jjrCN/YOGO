	...
	const bool debug)
{
  ...
  torch::Tensor view_first = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor view_alpha = torch::full({P}, 0, means3D.options().dtype(torch::kFloat32));
  ...
  if(P != 0)
  {
	  ...
	  auto tup = CudaRasterizer::Rasterizer::forward(
		...
		view_first.contiguous().data<int>(),
		view_alpha.contiguous().data<float>(),
		debug);
		
		rendered = std::get<0>(tup);
		num_buckets = std::get<1>(tup);
  }
  return std::make_tuple(rendered, num_buckets, out_color, out_invdepth, radii, view_first, view_alpha, geomBuffer, binningBuffer, imgBuffer, sampleBuffer);
}
