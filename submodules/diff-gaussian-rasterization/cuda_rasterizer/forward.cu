...
renderCUDA(
	...
	int32_t* __restrict__ view_first,
	float* __restrict__ view_alpha)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicAdd(&view_first[collected_id[j]], 1);
			atomicAdd(&view_alpha[collected_id[j]], alpha * T);
