...
ulimit -c 0
cp -r train.py "train_${exp_name_fusion}.py"

for i in ${!dir_name[@]}
do
    CUDA_VISIBLE_DEVICES="${cuda_device[$i]}" python "train_${exp_name_fusion}.py" \
        -s "$data_dir/${data_path_name[$i]}" \
        -d "$data_dir/${data_path_name[$i]}/depths" \
        --masks "$data_dir/${data_path_name[$i]}/masks" \
        -m "${output_dir_fusion[$i]}" \
        --port 359$((5+$i)) \
        ...
        --sensor_mod fusion \
        --bad_image_list "${output_dir_expo[$i]}/exclude_list.txt" &
done
wait
echo "训练完成: $exp_name_fusion"
