...
ulimit -c 0
cp -r train.py "train_${exp_name_expo}.py"

for i in ${!dir_name[@]}
do
    CUDA_VISIBLE_DEVICES="${cuda_device[$i]}" python "train_${exp_name_expo}.py" \
        ...
        --only_train_expo \
        --resolution 2 \
        --base_gaussian_dir "${output_dir_base[$i]}/point_cloud/point_cloud.ply"  &
done
wait
echo "训练完成: $exp_name_expo"
for i in ${!dir_name[@]}
do
    python tools/filter_expo.py --json "${output_dir_expo[$i]}/exposure.json" \
                                --out "${output_dir_expo[$i]}/exclude_list.txt" \
                                --replace
done
echo "剔除低质量完成: $exp_name_expo"
